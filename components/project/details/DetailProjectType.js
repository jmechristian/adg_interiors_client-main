...
const DetailProjectType = ({ name, detail, dividers, id }) => {
    ...
    <>
      {detail.data.length > 0 ? (
        <div className='flex flex-col mb-4 lg:pl-9 xl:pl-9' ref={lineRef}>
          ...
          <motion.div
            className='font-brand-serif text-xl md:text-lg lg:text-xl mt-0 detail-item w-full flex flex-wrap justify-start'
            ref={categoryRef}
            initial={{ opacity: 0 }}
            animate={lineInView && { opacity: 1 }}
          ...
          >
            {detail.data.map((item, index) => (
              <div key={index}>{item.attributes.type}</div>
            ))}
          </motion.div>
        </div>
      ) : (
...
export default DetailProjectType;
